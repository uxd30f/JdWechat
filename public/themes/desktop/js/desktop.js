!function () {
    ...
    var c = {
        setting: function (a) {
            ...
            b("#opening-menu").toggleClass("opening-menu-on").off("mousedown", c.stope).on("mousedown",
                c.stope);
            ...
            b(document).find(".taskbar-app").each(function (c, d) {
                b(d).attr("title") == e.title && (a.removeClass("disabled"), b(d).click(), m = !1)
            });
            if (m) {
                if (!c.maxApp()) {
                ...
                var g = e.width ? e.width : .8 * b(".desktop-container").width(),
                    k = e.height ? e.height : .9 * b(".desktop-container").height(), f = "", h = d.open({
                        type: 2,
                        ...
                        min: function (a, c) {
                            ...
                            var d = e.sort().pop();
                            b(document).find(".layui-layer-iframe:visible").each(function (a,
                                                                                           c) {
                ...
                1 ===
                e.full && d.full(h);
                f = "taskbar-layui-layer" + h;
                g = "";
                g = b(".taskbar-app span.desktop-title").hasClass("layui-hide") ? "layui-hide" : "";
                g = e.isicon ? "" + ('<div class="layui-inline layui-elip taskbar-app taskbar-app-on" title="' + e.title + '" id="' + f + '"><i class="layui-icon" style=" background-color:' + e.iconbg + '">' + e.icon + '</i><span class="desktop-title layui-elip ' + g + '">' + e.title + "</span></div>") : "" + ('<div class="layui-inline layui-elip taskbar-app taskbar-app-on" title="' + e.title + '" id="' + f + '"><span class="desktop-title layui-elip ' +
                    g + '">' + e.title + "</span></div>");
                b("#" + f).is(":visible") || (b(".desktop-taskbar-app-list").append(g), b("#" + f).on("click", function () {
                    var a = b(this);
                    a.hasClass("taskbar-app-on") ? b("#layui-layer" + h).find(".layui-layer-setwin .layui-layer-min").click() : (a.addClass("taskbar-app-on").siblings().removeClass("taskbar-app-on"), b("#layui-layer" + h).show(), d.zIndex = parseInt(d.zIndex + 1), d.style(h, {zIndex: d.zIndex}))
            ...
            var c = b(".desktopContainer"), e = 0, d = 0, k = 96, f = 96, h = 0, h = c.height() - 40;
            c.width();
            ...
            var a =
                    ["{{# layui.each(d.menu, function(index, menuitem){ if(index>=" + parseInt(desktpData.menu.length - 1) + ")return false;}}", '<div class="swiper-slide"><div class="desktopContainer"  data-menuid="{{menuitem.menuid}}" data-name="{{menuitem.name}}" >{{# layui.each(menuitem.app, function(index, app){}}<div class="desktop-app" data-id="{{d.apps[app].appid}} " data-title="{{d.apps[app].name}}" data-url="{{d.apps[app].url}}" data-icon="{{d.apps[app].icon}}" data-iconbg="{{d.apps[app].iconbg}}"  data-isicon="{{d.apps[app].isicon}}" data-height="{{d.apps[app].height}}" data-width="{{d.apps[app].width}}" data-fid="{{app}}" data-full="{{d.apps[app].full}}"><i class="{{d.apps[app].iconclass}}" style="background-color:{{d.apps[app].iconbg}}">{{d.apps[app].icon}}</i><span class="desktop-title layui-elip">{{d.apps[app].name}}</span></div>{{# });}}</div></div>{{# }); }} '].join(""),
                c = ["{{# layui.each(d.menu[" + parseInt(desktpData.menu.length - 1) + "].app, function(index, app){}}", '<div class="desktop-app" data-id="{{d.apps[app].appid}} " data-title="{{d.apps[app].name}}" data-url="{{d.apps[app].url}}" data-icon="{{d.apps[app].icon}}" data-iconbg="{{d.apps[app].iconbg}}"  data-isicon="{{d.apps[app].isicon}}" data-height="{{d.apps[app].height}}" data-width="{{d.apps[app].width}}" data-fid="{{app}}" data-full="{{d.apps[app].full}}"><i class="{{d.apps[app].iconclass}}" style="background-color:{{d.apps[app].iconbg}}">{{d.apps[app].icon}}</i><span class="desktop-title layui-elip">{{d.apps[app].name}}</span></div>{{# });}}'].join("");
            ...
            b(".desktopContainer").on("contextmenu", function (a) {
                ...
                a = a + d.height() >= k - 40 ? k - d.height() - 15 : a;
            ...
            d.show().find(".unlock-see-pwd").hover(function () {
                d.find(".unlock-pwd").attr("type", "text")
            }, function () {
